...
@Database(
    entities = [VehicleEntity::class, VehicleStatusEntity::class],
    version = 1,
    exportSchema = false,
)
...
fun VehicleEntity.toDomain() = Vehicle(
    vin = vin, vehicleId = vehicleId, nickname = nickname,
    modelName = modelName, modelYear = modelYear, modelCode = modelCode,
    licensePlate = licensePlate,
    fuelType = FuelType.valueOf(fuelType),
    region = BluelinkRegion.valueOf(region),
    generation = generation, customImageUri = customImageUri,
)

fun Vehicle.toEntity() = VehicleEntity(
    vin = vin, vehicleId = vehicleId, nickname = nickname,
    modelName = modelName, modelYear = modelYear, modelCode = modelCode,
    licensePlate = licensePlate, fuelType = fuelType.name, region = region.name,
    generation = generation, customImageUri = customImageUri,
)

fun VehicleStatusEntity.toDomain() = VehicleStatus(
    vin = vin, timestamp = timestamp, isLocked = isLocked,
    ...
    trunkOpen = trunkOpen, hoodOpen = hoodOpen, engineRunning = engineRunning,
    evBatteryPercent = evBatteryPercent, evRangeKm = evRangeKm,
    evCharging = evCharging, evChargeTargetPercent = evChargeTargetPercent,
    evPluggedIn = evPluggedIn, estimatedChargingMinutes = estimatedChargingMinutes,
    fuelLevelPercent = fuelLevelPercent, fuelRangeKm = fuelRangeKm,
    climateOn = climateOn, interiorTempC = interiorTempC,
    exteriorTempC = exteriorTempC, targetTempC = targetTempC,
    defrostFront = defrostFront, defrostRear = defrostRear,
...
fun VehicleStatus.toEntity() = VehicleStatusEntity(
    vin = vin, timestamp = timestamp, isLocked = isLocked,
    doorFrontLeft = doorFrontLeft.name, doorFrontRight = doorFrontRight.name,
    doorRearLeft = doorRearLeft.name, doorRearRight = doorRearRight.name,
    trunkOpen = trunkOpen, hoodOpen = hoodOpen, engineRunning = engineRunning,
    evBatteryPercent = evBatteryPercent, evRangeKm = evRangeKm,
    evCharging = evCharging, evChargeTargetPercent = evChargeTargetPercent,
    evPluggedIn = evPluggedIn, estimatedChargingMinutes = estimatedChargingMinutes,
