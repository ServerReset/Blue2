package com.blue2.app.automotive.screens
...
class HomeCarScreen(carContext: CarContext) : Screen(carContext) {
    ...
    override fun onGetTemplate(): Template {
        ...
        if (vehicles.isEmpty()) {
            return MessageTemplate.Builder("No vehicles found. Please check your Bluelink account.")
                .setTitle("Blue2")
                .build()
        }
        ...
        vehicles.forEach { vehicle ->
            ...
            listBuilder.addItem(
                Row.Builder()
                    ...
                    .addText(description)
                    .setOnClickListener { screenManager.push(CarControlScreen(carContext, vehicle, status)) }
                    .build()
            )
        }
...
class CarControlScreen(
    ...
    override fun onGetTemplate(): Template {
        ...
        paneBuilder.addRow(
            Row.Builder()
                .setTitle(if (status?.isLocked == true) "🔒 Vehicle is Locked" else "🔓 Vehicle is Unlocked")
        ...
        // Add pane rows for EV/fuel info
    ...
    private suspend fun refreshStatus() {
        repository.getVehicleStatus(vehicle.vin, forceRefresh = true).onSuccess { s ->
            status = s
        }
        isLoading = false
        invalidate()
