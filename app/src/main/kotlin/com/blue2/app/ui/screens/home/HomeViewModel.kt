...
class HomeViewModel @Inject constructor(
    ...
    fun refreshAll() {
        ...
        viewModelScope.launch {
            _state.update { it.copy(isRefreshing = true) }
            vehicles.forEach { v ->
                launch { repository.getVehicleStatus(v.vin, forceRefresh = true) }
            }
            _state.update { it.copy(isRefreshing = false) }
        }
    }
    ...
    fun startEngine(vin: String, climate: ClimateSettings? = null) =
        sendCommand(vin, "engine") { repository.startEngine(vin, climate) }
    fun stopEngine(vin: String) = sendCommand(vin, "engine") { repository.stopEngine(vin) }
    fun startClimate(vin: String, settings: ClimateSettings) =
        sendCommand(vin, "climate") { repository.startClimate(vin, settings) }
        ...
        viewModelScope.launch {
            ...
            action()
                .onSuccess { result ->
                    ...
                    _state.update { it.copy(successMessage = if (result.success) msg else null, error = if (!result.success) msg else null) }
    ...
    fun isCommandLoading(vin: String, cmd: String) =
        _state.value.commandLoading[commandKey(vin, cmd)] == true
